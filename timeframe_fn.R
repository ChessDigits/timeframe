...
# get acf at lags 1 and 2 for a vector
# helper fn
get_acf_lags_1_2 <- function(v) acf(v, lag.max=2, plot=F)$acf[2:3]


# generate df with acf with lags 1-2 as columns
get_df_acf_lags <- function(tt)
{
  "
  input: tt is list as generated by get_list_time_series_for_time_taken_for_each_game()
  output: df with lags as variables
  "
  tt_seq_of_times <- sapply(tt, \(m) as.vector(t(m)))
  a <- sapply(tt_seq_of_times, get_acf_lags_1_2) %>% t %>% as.data.frame # matrix; row 1 is lag 1, row 2 is lag 2
  colnames(a) <- c("Lag1", "Lag2")
